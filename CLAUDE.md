...
- **➕ Nuovo giocatore** (`adminCreatePlayer()`): name, surname, email, password, **telefono (required)**, club → calls the **`admin-create-user` Edge Function** (service role): creates the auth user with **email already confirmed and password set**, plus an **active** `pm_profiles` row at PR 35 (falls back to inserting the profile if the signup trigger didn't). The function accepts callers with `pm_profiles.role` `'admin'` (any club) or `'manager'` — **for managers the new player's club is FORCED server-side to their own club** (the client-sent value is ignored, so the UI lock cannot be bypassed). Anyone else gets 403; knowing the URL is useless. Source: `supabase/functions/admin-create-user/index.ts`.
  - **Consent attestation is mandatory**: the form has a required checkbox ("Confermo di aver fatto leggere e accettare i Termini/Privacy al giocatore") → sends `consents_attested:true`; the function rejects creation without it and records an **assisted consent** in the auth user_metadata with server timestamp, document version and the attestor's id/role (see Consensi below). It deliberately does NOT fake a player click — it records who attested the collection.
...
- **Consensi (Termini/Privacy/18+)**: recorded in `auth.users.raw_user_meta_data.consents` (NOT in `pm_profiles` — read it via SQL editor/service role only) as `{mode, maggiorenne, termini, privacy, version, at, ...}`. Two modes, always truthful about HOW consent was collected:
  - `mode:'self'` — set by `doRegister()` at signup (the player ticked the 3 checkboxes themselves; before this existed the form required the ticks but stored nothing).
  - `mode:'assisted'` — set server-side by the `admin-create-user` Edge Function when admin/manager creates the account, with `attested_by`/`attested_role` from the caller's JWT. The ~14 players created directly before the feature were marked retroactively via a one-off SQL update (note `'attestazione retroattiva'`), attested by the admin.
  - Never fabricate a `self` consent for a player who didn't click — an assisted attestation is the honest (and legally meaningful) record for accounts created on someone's behalf.
...
- `sw.js` — Service worker, current version `padelmeeting-v88`; precaches `index.html` + icons + `supabase.js`. **Mixed caching strategy** (do not revert to blanket cache-first):
...
- `manifest.json` — PWA manifest; icons use `"purpose": "any"` (not maskable) to avoid Android adaptive cropping. `background_color` is `#02191B` — the exact background of the icon PNGs — so the Android splash is uniform (no visible box around the logo); `theme_color` stays `#0E2723` (app UI/status bar). Installed WebAPKs pick up manifest/icon changes lazily (days, or on reinstall)
